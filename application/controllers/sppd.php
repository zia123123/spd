...
class sppd extends CI_Controller{
    ...
    function index()
    {
		...
		$query = "select *,
				  ...
				  where STR_TO_DATE(a.tgl_sppd, '%d-%m-%Y') BETWEEN '2019-01-01' and '2050-12-31'
				  GROUP BY a.id_sppd
    ...
    function post()
    {
        if(isset($_POST['submit']))
        {
			...
			$this->form_validation->set_rules('jenis_plafond', 'Jenis Plafond','trim|required');
			...
			$from	= date('Y-m-d', strtotime($date_awal));
			$to		= date('Y-m-d', strtotime($date_akhir));
			...
			if ($this->form_validation->run() === FALSE) {
				...
				$cek_dinas_db = $this->db->query("select a.nama_pelaksana, b.tgl_keberangkatan, b.tgl_kepulangan 
											from tb_pelaksana a inner join tb_sppd b on a.id_sppd = b.id_sppd
											where a.noreg='$ketua_plk_tugas' and 
											( 
											STR_TO_DATE(b.tgl_keberangkatan, '%d-%m-%Y') <= '$from' && STR_TO_DATE(b.tgl_kepulangan, '%d-%m-%Y') >= '$to' or 
											STR_TO_DATE(b.tgl_keberangkatan, '%d-%m-%Y') BETWEEN '$from' and '$to' or 
											STR_TO_DATE(b.tgl_kepulangan, '%d-%m-%Y') BETWEEN '$from' and '$to'
											)");
				
				if($cek_dinas_db->num_rows()>0)
				{
				...
				else
				{
					...
					foreach ($plk_tugas as $noreg){
						...
						$cek_dinas_db_2 = $this->db->query("select a.nama_pelaksana, b.tgl_keberangkatan, b.tgl_kepulangan 
											from tb_pelaksana a inner join tb_sppd b on a.id_sppd = b.id_sppd
											where a.noreg='$noreg' and 
											( 
											STR_TO_DATE(b.tgl_keberangkatan, '%d-%m-%Y') <= '$from' && STR_TO_DATE(b.tgl_kepulangan, '%d-%m-%Y') >= '$to' or 
											STR_TO_DATE(b.tgl_keberangkatan, '%d-%m-%Y') BETWEEN '$from' and '$to' or 
											STR_TO_DATE(b.tgl_kepulangan, '%d-%m-%Y') BETWEEN '$from' and '$to'
						...
						foreach($cek_dinas_db_2->result() as $r){
							$nama = $r->nama_pelaksana;
						}
						
						if($cek_dinas_db_2->num_rows()>0) {
							echo '<script type="text/javascript">alert("Input SPPD Gagal ! '.$nama.' masih dalam masa dinas !");window.history.go(-1);</script>';
							exit;
						} 
						//echo $cek_dinas_db_2->num_rows(); exit;
						
						/*
						$cek_dinas_plk = $this->db->query("select a.nama_pelaksana, a.noreg, b.tgl_keberangkatan,b.tgl_kepulangan from tb_pelaksana a inner join tb_sppd b on a.id_sppd = b.id_sppd where a.noreg = '$noreg'")->result();
						...
						foreach ($plk_tugas as $noreg){
							...
							if ($this->db->affected_rows($insertsppd) == 1) {
								...
								foreach($get_attr_kta_plk as $attr){
									$this->mcrud->manualQuery('INSERT INTO tb_pelaksana (noreg, nama_pelaksana, gol, jabatan, unit_kerja, id_sppd, status_pelaksana, jenjang, nip) values ("'.$ketua_plk_tugas.'","'.$attr->nama_pegawai.'","'.$attr->golongan.'","'.$attr->jabatan.'","'.$attr->unitkerja.'","'.$last_id_sppd.'","1","'.$attr->jenjang.'","'.$attr->nip.'")');
								}
								
								foreach ($plk_tugas as $noreg){
									$get_attr_plk_tgs = $this->db->query("select * from tb_pegawai where noreg='$noreg'")->result();
									foreach($get_attr_plk_tgs as $attr){
										$this->mcrud->manualQuery('INSERT INTO tb_pelaksana (noreg, nama_pelaksana, gol, jabatan, unit_kerja, id_sppd, status_pelaksana, jenjang, nip) values ("'.$noreg.'","'.$attr->nama_pegawai.'","'.$attr->golongan.'","'.$attr->jabatan.'","'.$attr->unitkerja.'","'.$last_id_sppd.'","0","'.$attr->jenjang.'","'.$attr->nip.'")');
									}
								}
        ...
        if(isset($_POST['submit']))
        {
			...
			*/
				foreach ($plk_tugas as $noreg){
					...
					$get_attr_kta_plk = $this->db->query("select * from tb_pegawai where noreg='$ketua_plk_tugas'")->result();
					foreach($get_attr_kta_plk as $attr){
						$this->mcrud->manualQuery('INSERT INTO tb_pelaksana (noreg, nama_pelaksana, gol, jabatan, unit_kerja, id_sppd, status_pelaksana, jenjang, nip) values 
												 ("'.$ketua_plk_tugas.'","'.$attr->nama_pegawai.'","'.$attr->golongan.'","'.$attr->jabatan.'","'.$attr->unitkerja.'","'.$id.'","1","'.$attr->jenjang.'","'.$attr->nip.'")');
					}
					...
					foreach ($plk_tugas as $noreg){
						...
						foreach($get_attr_plk_tgs as $attr){
							$this->mcrud->manualQuery('INSERT INTO tb_pelaksana (noreg, nama_pelaksana, gol, jabatan, unit_kerja, id_sppd, status_pelaksana, jenjang,nip) values 
													 ("'.$noreg.'","'.$attr->nama_pegawai.'","'.$attr->golongan.'","'.$attr->jabatan.'","'.$attr->unitkerja.'","'.$id.'","0","'.$attr->jenjang.'","'.$attr->nip.'")');
						}
					}
